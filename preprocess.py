...
def __parse_midi(data_fn):
    ...
    for m in melody_voice:
        print(m, m.quarterLength, m.offset)
    ...
    #print('melody_voice',len(melody_voice),melody_voice)
    #melody_voice.show('text')
    ...
    comp_stream.append([j.flat for i, j in enumerate(midi_data) 
        if i in partIndices])
    #print('comp_stream',len(comp_stream),comp_stream)
    ...
    #print('full_stream',len(full_stream),full_stream)
    ...
    #print('solo_stream',len(solo_stream),solo_stream)
    #for i,m in enumerate(solo_stream):
    #    print(i,m,len(m))
    ...
    print('melody_stream',len(melody_stream),melody_stream)
    #melody_stream.show('text')
    measures = OrderedDict()
    for i,m in enumerate(melody_stream):
        print(i,m.offset,m)
    offsetTuples = [(int(n.offset / 4), n) for n in melody_stream]
    print('offsetTuples',len(offsetTuples),offsetTuples)
    ...
    #print('chordStream',len(chordStream),chordStream)
...
def get_musical_data(data_fn):
    ...
    print('measures',len(measures),measures)
    print('chords',len(chords),chords)
    abstract_grammars = __get_abstract_grammars(measures, chords)
    print('abstract_grammars',len(abstract_grammars),abstract_grammars)
